...
describe('BookService', () => {
  ...
  const mockGenreService = { get: jest.fn().mockImplementation(() => mockGenre) };
  ...
  describe('addGenre / excludeGenre', () => {
    it('normal', async () => {
      await expect(service.addGenre(mockBook.authorId, mockBook.id, 'Fantasy')).resolves.toBeUndefined();
      await expect(service.excludeGenre(mockBook.authorId, mockBook.id, 'Fantasy')).resolves.toBeUndefined();
    });

    it('not author', async () => {
      await expect(service.addGenre(0, mockBook.id, 'Fantasy')).rejects.toThrow(ForbiddenException);
      await expect(service.excludeGenre(0, mockBook.id, 'Fantasy')).rejects.toThrow(ForbiddenException);
    });

    it('not author + admin', async () => {
      await expect(service.addGenre(0, mockBook.id, 'Fantasy', true)).resolves.toBeUndefined();
      await expect(service.excludeGenre(0, mockBook.id, 'Fantasy', true)).resolves.toBeUndefined();
    });
    ...
    it('user not found', async () => {
      ...
      await expect(service.addGenre(mockBook.authorId, mockBook.id, 'Fantasy')).rejects.toThrow(NotFoundException);
      await expect(service.excludeGenre(mockBook.authorId, mockBook.id, 'Fantasy')).rejects.toThrow(NotFoundException);
    });

    it('genre not found', async () => {
      mockGenreService.get.mockImplementationOnce(() => { throw new NotFoundException(); })
